...
object Spark extends App {
  ...
  println(KVcharacters.join(keyedChars).count())
  println(KVcharacters.leftOuterJoin(keyedChars, outputPartitions).collect().mkString("Array(", ", ", ")"))
  println(keyedChars.count())
  println(KVcharacters.coalesce(3).getNumPartitions)
  println()
